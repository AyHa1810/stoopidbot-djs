...
module.exports = async (client, message) => {
  ...
  // you can write anything after this to do anything everytime someone messages
  ...
  */
  ...
  do {
    ...
    var match = regexp.exec(message.content.replace(prefix, ''));
    if (match != null)
    {
  ...
  const command = args.shift().toLowerCase(); //shifts the args to all lower case
...
This command requires level ${container.levelCache[cmd.conf.permLevel]} (${cmd.conf.permLevel})` */ `You don\'t have permissions to use this command`);
    } else {
      return;
    }
  }
